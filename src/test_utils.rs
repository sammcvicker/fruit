...
impl TestRepo {
    ...
    pub fn init_git(&mut self) {
        ...
        Command::new("git")
            ...
            .unwrap_or_else(|e| {
                panic!(
                    "TestRepo::init_git: 'git init' failed at {:?}: {}",
                    repo_path, e
                )
            });
        ...
        Command::new("git")
            ...
            .unwrap_or_else(|e| {
                panic!(
                    "TestRepo::init_git: 'git config user.email' failed at {:?}: {}",
                    repo_path, e
                )
            });
        ...
        Command::new("git")
            ...
            .unwrap_or_else(|e| {
                panic!(
                    "TestRepo::init_git: 'git config user.name' failed at {:?}: {}",
                    repo_path, e
                )
            });
    ...
    pub fn add_file(&self, path: &str, content: &str) -> PathBuf {
        ...
        if let Some(parent) = full_path.parent() {
            fs::create_dir_all(parent).unwrap_or_else(|e| {
                panic!(
                    "TestRepo::add_file: failed to create parent directories for {:?}: {}",
                    full_path, e
                )
            });
        }
        fs::write(&full_path, content).unwrap_or_else(|e| {
            panic!("TestRepo::add_file: failed to write {:?}: {}", full_path, e)
        });
    ...
    pub fn add_untracked(&self, path: &str, content: &str) -> PathBuf {
        ...
        if let Some(parent) = full_path.parent() {
            fs::create_dir_all(parent).unwrap_or_else(|e| {
                panic!(
                    "TestRepo::add_untracked: failed to create parent directories for {:?}: {}",
                    full_path, e
                )
            });
        }
        fs::write(&full_path, content).unwrap_or_else(|e| {
            panic!(
                "TestRepo::add_untracked: failed to write {:?}: {}",
                full_path, e
            )
        });
        full_path
    ...
    pub fn stage_all(&self) {
        if self.git_initialized {
            ...
            Command::new("git")
                ...
                .unwrap_or_else(|e| {
                    panic!(
                        "TestRepo::stage_all: 'git add .' failed at {:?}: {}",
                        repo_path, e
                    )
                });
        }
    ...
    pub fn commit(&self, message: &str) {
        assert!(
            ...
            "TestRepo::commit: git not initialized - call init_git() first"
        );
        let repo_path = self.dir.path();
        Command::new("git")
            ...
            .unwrap_or_else(|e| {
                panic!(
                    "TestRepo::commit: 'git commit -m {:?}' failed at {:?}: {}",
                    message, repo_path, e
                )
            });
    }
